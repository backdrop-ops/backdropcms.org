 ...
 *
 */
class CRM_Sendgrid_Form_Report_Spam extends CRM_Report_Form {
  ...
  function select() {
    ...
    if (CRM_Utils_Array::value('charts', $this->_params)) {
      $select[] = "COUNT(civicrm_mailing_event_spam_report.id) as civicrm_mailing_spam_count";
      $this->_columnHeaders["civicrm_mailing_spam_count"]['title'] = ts('Spam Report Count');
    }
  ...
  function from() {
    ...
    $this->_from .= "
        ...
        INNER JOIN civicrm_mailing_event_spam_report
          ON civicrm_mailing_event_spam_report.event_queue_id = civicrm_mailing_event_queue.id
  ...
  function buildChart(&$rows) {
    ...
    $chartInfo = array('legend' => ts('Mail Spam Report'),
      'xname' => ts('Mailing'),
      'yname' => ts('Spam Reported'),
      'xLabelAngle' => 20,
      'tip' => ts('Mail Spam Reported: %1', array(1 => '#val#')),
    );
    foreach ($rows as $row) {
      $chartInfo['values'][$row['civicrm_mailing_mailing_name_alias']] = $row['civicrm_mailing_spam_count'];
    }
