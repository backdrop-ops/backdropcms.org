 ...
 * This file has common settings for CiviCRM
 * Sensitive information such as passwords are stored in civicrm.settings.local.php
 * See example.civicrm.settings.local.php for instructions.
 */
...
$cms_root = dirname(dirname(__DIR__));

/**
 * General database Settings
 */
$db = [];
foreach (['civicrm', 'backdrop'] as $app) {
  if (array_key_exists($app, $database_settings)) {
    $db[$app] = $database_settings[$app];
  } else {
    $db[$app] = $database_settings;
  }

  if (array_key_exists('port', $db[$app]) and !empty($db_port)) {
    $db[$app]['port'] = ':' . $db[$app]['port'];
  } else {
    $db[$app]['port'] = '';
  }
}
...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
  define( 'CIVICRM_UF_DSN'           , "mysql://{$db['backdrop']['user']}:{$db['backdrop']['password']}@{$db['backdrop']['host']}{$db['backdrop']['port']}/{$db['backdrop']['database']}?new_link=true");
}
...
if (!defined('CIVICRM_DSN')) {
  if (CIVICRM_UF === 'UnitTests' && isset($GLOBALS['_CV']['TEST_DB_DSN'])) {
    ...
    define('CIVICRM_DSN', "mysql://{$db['civicrm']['user']}:{$db['civicrm']['password']}@{$db['civicrm']['host']}{$db['civicrm']['port']}/{$db['civicrm']['database']}?new_link=true");
  }
}
...
// define( 'CIVICRM_MAIL_LOG', '/var/www/test/backdropcms/www/files/civicrm/templates_c//mail.log');
// }
