...
/**
 * @file
 * API documentation for Akismet module.
 */

/**
 * @defgroup akismet_api Akismet API
 * @{
 * Functions to integrate with Akismet form protection.
 *
 ...
 *
 * The Akismet API therefore supports two types of integration:
 * - Entity form integration: Akismet integrates with the add/edit form for an
 *   entity, and additionally with the delete confirmation form of the entity
 *   to send feedback to Akismet. Almost everything happens in an automated way,
 *   solely based on the information provided via Akismet's info hooks, as
 *   explained below.
 * - Free integration: Akismet integrates with a given form_id without 'entity'.
 *   Allowing users to send feedback requires to manually add "report to Akismet"
 ...
 *   found in contact_akismet_form_list(), akismet_mail_alter(), and related
 ...
 * be possible to be protected by Akismet:
 * @code
 ...
 * corresponding delete confirmation form via hook_akismet_form_list():
 *
 * @code
 * function im_akismet_form_list() {
 *   $forms['im_message_form'] = array(
 *     'title' => t('Instant messaging form'),
 *     'entity' => 'im',
 *     // Specify the $form_id of the delete confirmation form that allows
 *     // privileged users to delete a stored message. Akismet will automatically
 *     // add form elements to send feedback to Akismet to this form.
 ...
 * via hook_akismet_form_list(). All details about the form are only required and
 * asked for, if the site administrator actually enables Akismet's protection for
 * the form. Therefore, everything else is registered via
 * hook_akismet_form_info():
 *
 * @code
 * function im_akismet_form_info($form_id) {
 *   switch ($form_id) {
 *     case 'im_message_form':
 *       $form_info = array(
 *         // Optional: User permission list to skip Akismet's protection for.
 ...
 *         'moderation callback' => 'im_akismet_form_moderation',
 ...
 *         // are predefined data properties sent to Akismet (see full list in
 *         // hook_akismet_form_info()), the values refer to field keys in
 ...
 *           // Important: The following assignment means that Akismet is able to
 ...
 * @code
 * function im_akismet_form_moderation(&$form, &$form_state) {
...
/**
 * Return information about forms that can be protected by Akismet.
 *
 * Akismet invokes this hook for all modules to gather information about forms
 * that can be protected. Only forms that have been registered via this hook are
 * configurable in Akismet's administration interface.
 *
 * @return
 ...
 *     constructor function for 'entity'. Akismet automatically adds the
 *     "Report as inappropriate" options to this confirmation form. Requires a
 *     'post_id' mapping via hook_akismet_form_info(). Requires the delete
 ...
 *     the entity ID. For example, user_akismet_form_list() specifies
 *     'user/%id/cancel'.
 ...
 *     access to Akismet's dedicated "report to Akismet" form, which should return
 ...
 *     entity after reporting it to Akismet.  The callback will receive the
 ...
 */
function hook_akismet_form_list() {
  // Mymodule's comment form.
  $forms['mymodule_comment_form'] = array(
    'title' => t('Comment form'),
    'entity' => 'mymodule_comment',
    // Akismet does not know how to determine access and the callback to invoke
    // for reporting and deleting the entity, so your module needs to manually
    // output links to Akismet's generic "Report to Akismet" form on
    // 'akismet/report/[entity]/[id]' and supply the following two callbacks.
  ...
  $forms['mymodule_user_register'] = array(
    ...
    // Akismet will automatically integrate with the delete confirmation form and
    // send feedback for the 'entity' specified above and the 'post_id'
    // specified via hook_akismet_form_info(). The delete confirmation form has
...
/**
 * Alter the list of forms that can be protected by Akismet.
 *
 * @param &$form_list
 *   An associative array containing information about the forms that can be
 *   protected, keyed by $form_id. See hook_akismet_form_list() for details.
 */
function hook_akismet_form_list_alter(&$form_list) {
...
/**
 * Return information about a form that can be protected by Akismet.
 *
 * @param $form_id
 ...
 *     If omitted, Akismet can only provide a CAPTCHA protection for the form.
 ...
 *     sent to Akismet for validation. The submitted form values of all mapped
 *     elements are not used for the post's body, so Akismet can validate certain
 ...
 */
function hook_akismet_form_info($form_id) {
  switch ($form_id) {
    // Mymodule's comment form.
...
/**
 * Alter registered information about a form that can be protected by Akismet.
 *
 * @param &$form_info
 *   An associative array describing the protectable form. See
 *   hook_akismet_form_info() for details.
 * @param $form_id
 *   The $form_id of the form.
 */
function hook_akismet_form_info_alter(&$form_info, $form_id) {
